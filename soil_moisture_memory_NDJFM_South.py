...
"""
Soil Moisture Memory for Southern Region (NDJFM)

This script computes random error corrected soil moisture memory (SMM) and
measurement error metrics from seasonal soil moisture residuals for the
Southern region (60°S-23°S) during November-March (NDJFM) by:
...
Regional Specifics:
  - Region: Southern (60°S-23°S)
  - Season: November-March (NDJFM)
  - Time period: 1979-2023
  - Data source: Climate Change Initiative (CCI) soil moisture residuals

Usage:
    python soil_moisture_memory_NDJFM_South.py
"""
...
def estimate_memory_parameter(exponential_fits):
    ...
    Uses the relationship SMM = (-1 - intercept) / slope, bounded to [0, 151] days.
    ...
    return min(memory_parameter, 151.0)  # Upper bound: 151 days
...
def load_regional_data(season_files, latitude_min, latitude_max, months_to_select):
    ...
    season_codes = sorted({month_to_season[m] for m in months_to_select})
...
def analyze_and_save_results(dataset, output_file_path, region_description=""):
    ...
    output_dataset["SMM"].attrs.update({
        "long_name": f"Random error corrected soil moisture memory (SMM) for {region_description}",
    ...
    output_dataset.attrs.update({
        ...
        "Description": (
            f"Random error corrected soil moisture memory (SMM), relative measurement error (δ/σ), "
...
def main():
    ...
    for the Southern region (60°S-23°S) during NDJFM season.
    ...
    # Regional parameters - Southern Region (60°S-23°S), NDJFM
    LATITUDE_MIN = -23
    LATITUDE_MAX = -60
    MONTHS_SELECTED = [11, 12, 1, 2, 3]  # November-March (NDJFM)
    REGION_DESCRIPTION = "November-March (NDJFM) over 60°S-23°S Southern region"
