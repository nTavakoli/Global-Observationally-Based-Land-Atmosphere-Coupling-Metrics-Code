...
"""
Soil Moisture Memory for Error Parameters - Tropical Region (All Months)
...
Tropical region (23°S-23°N) across all months (year-round) by:
...
Regional Specifics:
  - Region: Tropical (23°S-23°N)
  - Season: All months (year-round)
  - Time period: 1979-2023
  - Data source: Climate Change Initiative (CCI) soil moisture residuals

Usage:
    python soil_moisture_memory_All_Months_Tropics.py
"""
...
def estimate_memory_parameter(exponential_fits):
    ...
    Uses the relationship SMM = (-1 - intercept) / slope, bounded to [0, 365] days.
    ...
    return min(memory_parameter, 365.0)  # Upper bound: 365 days
...
def main():
    ...
    for the Tropical region (23°S-23°N) across all months.
    ...
    OUTPUT_FILE = os.path.join(OUTPUT_DIRECTORY, 'SMM_CCI_Tropics_AllMonths_0.25.nc')
    
    # Regional parameters - Tropical Region (23°S-23°N), all months
    LATITUDE_MIN = 23
    LATITUDE_MAX = -23
    MONTHS_SELECTED = list(range(1, 13))  # All months (1-12)
    REGION_DESCRIPTION = "All months (year-round) over 23°S-23°N Tropical region"
